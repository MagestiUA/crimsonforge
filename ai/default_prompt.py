...
def get_default_system_prompt(source_lang: str, target_lang: str) -> str:
    return (
        ...
        f"6. For character names, place names, and proper nouns: if {target_lang} uses a different script "
        f"than the source (e.g. Cyrillic vs Latin), transliterate the name phonetically into {target_lang}'s "
        f"script and spelling conventions - a name left in the wrong alphabet reads as a bug, not a deliberate "
        f"choice. If {target_lang} shares the source script, keep the name as-is unless there is an "
        f"established official localization. Either way, transliterate/adapt the SOUND and spelling only - "
        f"never invent a translated meaning for a name.\n"

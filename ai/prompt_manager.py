...
class PromptManager:
    ...
    def __init__(self, system_prompt_template: str = "", user_prompt_template: str = ""):
        self._default_system_template = (
            ...
            "6. For character names, place names, and proper nouns: if {target_lang} uses a different script "
            "than the source (e.g. Cyrillic vs Latin), transliterate the name phonetically into {target_lang}'s "
            "script and spelling conventions - a name left in the wrong alphabet reads as a bug, not a deliberate "
            "choice. If {target_lang} shares the source script, keep the name as-is unless there is an "
            "established official localization. Either way, transliterate/adapt the SOUND and spelling only - "
            "never invent a translated meaning for a name.\n"

...
CHANGELOG: list[tuple[str, str, list[str]]] = [
    ...
    (
        "1.29.0", "2026-07-05", [
            ...
            "[Translation] Stop is now responsive even with many concurrent lanes in flight. `AIProviderBase.request_stop()`/`reset_stop()` plus interruptible retry-backoff waits in the Gemini and OpenAI-compatible providers mean a lane stuck in a 429/5xx retry sleep wakes up immediately instead of blocking Stop for up to 30 seconds per lane. Verified live: calling stop() while 5 lanes were mid-run returned in ~3.6s (bounded by already-in-flight HTTP requests finishing, which can't be cancelled mid-request) instead of minutes.",
            "[Translation] Cross-language reference context for Ukrainian targets. English alone doesn't mark grammatical gender/case/number, a documented weakness in at least one existing community Ukrainian translation for this game. When the target language is Ukrainian, Auto-Translate-All now reads the game's Korean (original source language) and Russian (grammatically close to Ukrainian) text for the same line and passes both into the AI prompt as disambiguation-only reference - translate from the English source, use Korean when the English text itself seems ambiguous, use Russian only to pick correct grammatical agreement, never copy its wording. Silently skipped if the game or those languages aren't available.",
            "[Tooling] New `tools/flag_latin_leak_entries.py` — one-off quality pass that finds 'translated' entries whose translation still contains unprotected Latin-script text (names left untranslated before the transliteration prompt fix, or a rare batch response that echoed source text back unchanged) and requeues them to PENDING with the old translation stashed in notes. Correctly ignores protected placeholder tokens (including their own sentinel spelling, which contains Latin letters) and Roman numerals (a legitimate kept-as-is convention for item/blueprint tiers). Verified against a real 187k-entry project: an initial naive version flagged 22,291 entries almost entirely as false positives from the sentinel format itself; the corrected version flags 5,625, predominantly genuine leaked names on manual review.",
        ],
    ),
    (
        "1.28.0", "2026-07-04", [
            "[Translation] Project storage moved from a single JSON file to SQLite (`translation/translation_db.py`). Full save of a 187k-entry project drops from ~4.17s to ~1.5s and the file itself shrinks (~48MB vs ~165MB in testing; the user's real 187k-entry project went from 245MB to 135MB). `TranslationProject.persist_entry()` does a single-row upsert directly into the same file during a running batch, so `translation/checkpoint_journal.py` (a separate SQLite journal that existed purely to survive a crash between full JSON saves) is no longer needed and has been removed. `TranslationProject.load()` auto-detects a legacy JSON project by sniffing for the SQLite magic header and migrates it to SQLite in place — the original JSON is preserved as a `.legacy-json-backup` sidecar and the new file is built at a temp path and atomically swapped in, so an interrupted migration can never leave a project file that's neither valid JSON nor valid SQLite.",
            "[Translation] New 'Patch Target (game file)' selector, independent from Source Language. Previously 'Patch to Game' always wrote translated text back into the same file it read as source, which makes that file useless as a future comparison baseline the moment it's patched once - a later sync could read its own prior output back as if it were new source text. The new selector lets the destination be any discovered game language file; it defaults to whichever file matches Destination Language, and leaves nothing preselected when the game has no native file for the target language (e.g. a language the game doesn't ship), forcing an explicit choice of which existing slot should carry the translation - mirroring the 'Game Language to Replace' concept `ui/dialogs/ship_dialog.py` already exposed for packaged mod exports.",
            "[Translation] Source-text change detection now resets the affected entry straight to PENDING (stashing the previous translation in `notes` for reference) instead of leaving it TRANSLATED under an implicit 'needs re-review' label a human had to notice. The next Auto-Translate-All run picks it back up automatically since it only processes PENDING entries. Locked entries (auto-approved placeholders) are left untouched.",
            "[Translation] Character names, place names, and other proper nouns are now transliterated into the target language's own script when it differs from the source (e.g. Latin -> Cyrillic), instead of being left untranslated in the source script - a name left in the wrong alphabet reads as a bug, not a deliberate choice. Same-script pairs keep the old behavior (preserve as-is unless there's an established official localization).",
            "[Tooling] New `tools/repair_corrupted_original_text.py` — one-off repair for projects affected by the pre-1.28 bug where a source file that had already been patched once could get read back as fresh source text, silently overwriting `original_text` with the entry's own prior translation. Repairs affected entries against the untouched `BaselineManager` record (which only ever records a key's value once and never overwrites it) without touching `translated_text` or status, so existing translations are never discarded or needlessly re-queued. Verified against a real 187k-entry project: 150,387 entries (80%) had this corruption, all repaired cleanly with zero ambiguous cases and zero translation/status changes.",
        ],
    ),
    (
        "1.27.0", "2026-07-04", [
            "[Translation] Crash-safe checkpoint journal (`translation/checkpoint_journal.py`) replaces the old throttled full-JSON-rewrite autosave. Every translated entry is upserted into a small SQLite journal (~35ms) instead of rewriting the whole project file (4.17s measured on a 187k-entry/165MB project). The journal is merged back into a project on load and cleared after a successful full save, so an interrupted run never loses already-translated entries. `TranslationProject.save()` now holds a lock for its whole body, fixing a WinError 32 race between the periodic autosave timer and per-item checkpoints.",
            "[Translation] Duplicate-aware batch translation — `TranslationBatchProcessor._dedup_and_translate` groups pending entries by identical `original_text`, translates each unique string once, and stamps the result across every occurrence. One measured project had 'Cannot be used at the moment.' repeated 1,481 times among ~40k pending strings (~51% overall duplicate rate), so this roughly halves real AI calls on typical localization data.",
            "[Translation] JSON-array batch requests replace one-string-per-call translation. `ai/translation_engine.py` groups pending strings into chunks (bounded by batch size and a token budget) and sends/receives them as a single JSON object per chunk. Any chunk whose response can't be trusted (broken JSON, wrong item count, duplicate/missing ids) is bisected and retried in halves instead of falling back to one call per item immediately — isolates the one bad item in ~log2(n) calls instead of paying for n calls every time. Default batch size raised 10 -> 20 based on measurement (20 was fast and reliable on Ollama + Gemma4; 100 effectively stalled).",
            "[Translation] Gemini and OpenAI-compatible providers now retry 429 with a real 30s cooldown (free-tier RPM quotas reset per minute) instead of treating rate limiting as broken content worth bisecting, and retry 5xx with a short 5s backoff since that's the backend's fault, not the request's. Both now treat an empty AI response as a failure instead of silently accepting it as a successful empty translation.",
            "[Translation] Gemini `list_models()` now filters by `generateContent` capability instead of a `gemini` substring match, which was wrongly excluding Gemma models available on the same API. Stopped setting `max_output_tokens` on Gemini calls — Gemma reasoning models spend output tokens on internal 'thinking' that counts against that budget but can't be sized in advance, and explicitly setting `thinking_budget` is rejected outright for these models. Disabled the SDK's own internal HTTP retry (5 attempts, backoff to 60s per attempt) since it retried the same status codes our own retry logic already handles, and the two stacking together could silently retry for minutes before our code ever saw the error.",
            "[Ollama] CrimsonForge now force-restarts any Ollama server it didn't start itself before a translation batch, so KV-cache quantization (q4_0) and flash attention always apply — a server left running unquantized before CrimsonForge starts otherwise silently ignores those environment variables for its whole lifetime. Fixes an orphaned-VRAM bug where killing only `ollama.exe` left `llama-server.exe` (the actual model process) alive holding GPU memory. New `prepare_for_batch()` / `ensure_ready()` provider hooks let Ollama verify or restart its backend before a batch run and when resuming a long pause. Reasoning/thinking is always disabled for translation requests; context window is fixed at 64k rather than sized dynamically per batch (dynamic sizing caused empty-response failures at larger batch sizes).",
            "[Tooling] New `tools/import_legacy_translations.py` migrates a Ukrainian translation memory from an older, separate SQLite-based translation tool into a CrimsonForge project — matches by current in-game key, imports translations whose English source is unchanged, stashes stale ones (source text changed since) in notes for reference, and leaves new keys pending.",
            "[Tooling] New `tools/bench_batch_translate.py` benchmarks batch-size/latency/fallback-rate tradeoffs against a live provider, used to pick the current default batch size.",
        ],
    ),
    (
        "1.26.0", "2026-05-12", [
